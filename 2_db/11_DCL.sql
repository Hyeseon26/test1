...
GRANT CREATE SESSION TO jhs_sample; -- Grant을(를) 성공했습니다.
...
GRANT CREATE TABLE TO jhs_sample; -- Grant을(를) 성공했습니다.
ALTER USER jhs_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
--> jhs_sample 계정에서 jhs 계정의 EMPLOYEE 테이블 조회

-- 1. [샘플 계정] jhs계정의 EMPLOYEE 테이블 조회
SELECT * FROM jhs.EMPLOYEE; --> EMPLOYEE 테이블(객체) 조회 권한 없음

-- 2. [이니셜 계정] 샘플 계정에 EMPLOYEE 테이블 조회 권한 부여
GRANT SELECT ON EMPLOYEE TO jhs_sample; -- Grant을(를) 성공했습니다.

-- 3. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM jhs.EMPLOYEE;

-- 4. [이니셜 계정] 부여했던 SELECT 권한 회수
REVOKE SELECT ON EMPLOYEE FROM jhs_sample;

-- 5. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM jhs.EMPLOYEE; --> 조회 안됨
