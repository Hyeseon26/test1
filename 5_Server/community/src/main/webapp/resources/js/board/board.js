...
(function(){
    ...
    if(goToListBtn != null){ // 목록으로 버튼이 화면에 있을 때만 이벤트 추가
        ...
        goToListBtn.addEventListener("click", function(){
            ...
            // http://localhost:8080/community/board/detail?no=449&cp=6&type=1
...
(function(){
    ...
    if(deleteBtn != null){ // 삭제 버튼이 화면에 존재할 때
        deleteBtn.addEventListener("click", function(){
            // 현재 : detail?no=1601&cp=1&type=1

            // 목표 : delete?no=1601&type=1
            ...
            const no = "?no=" + params.get("no"); // ?no=1601

            const type = "&type=" + params.get("type"); // &type=1

            // url에 쿼리스트링 추가
            url += no + type; // delete?no=1601&type=1
...
(function(){
    ...
    if(select != null){ // 검색창 화면이 존재할 때만 코드 적용
        ...
        // option을 반복접근해서 value와 key가 같으면 selected 속성 추가
